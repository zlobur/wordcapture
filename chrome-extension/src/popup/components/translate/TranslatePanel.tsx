...
export function TranslatePanel({ langFrom, langTo, onChangeLangFrom, onChangeLangTo }: Props) {
  ...
  const enrichAbort = useRef(0);
  const suppressAutoRef = useRef(false);
  const effectiveLangsRef = useRef({ from: langFrom, to: langTo });
  const { add: addCard } = useCards();
  effectiveLangsRef.current = { from: langFrom, to: langTo };
  ...
  const doTranslate = async (text: string, overrideFrom?: string, overrideTo?: string, skipAutoDetect?: boolean) => {
    ...
    try {
      ...
      if (generation !== enrichAbort.current) return;
  ...
  const handleSave = () => {
    if (!result) return;
    const resolvedFrom = effectiveLangsRef.current.from as LangCode;
    const resolvedTo = effectiveLangsRef.current.to as LangCode;
    addCard({
      ...
      deckId: null,
      sourceLang: resolvedFrom === "auto" as any ? "en" : resolvedFrom,
      targetLang: resolvedTo,
    });
    setSaved(true);
